...
def parse_commandline():
    ...
    parser.add_option("-o", "--outputDir", help="output file",default="../output/GW190814/CFHT")
    parser.add_option("-l","--transients", help="Transient list.", default='../data/GW190814/transients.dat')
    parser.add_option("-t","--transient", help="Transient name.", default='DG19wxnjc')
    parser.add_option("-d","--dataDir", help="Data directory.", default='../data/GW190814/DG19wxnjc_CFHT/') 
...
if opts.doKowalski:
    ...
    ra_hex = transients["ra"][idx][0]
    dec_hex = transients["dec"][idx][0]

    ra_transient  = Angle(ra_hex, unit=u.hour).deg
    dec_transient = Angle(dec_hex, unit=u.deg).deg
...
files_old = glob.glob(workdir_fits_red+'*.flt')
print(files_old)#,workdir_fits_red

for i in range(len(files_old)):
    if i in [108,113]: continue
    ...
    if (ra_min > ra_transient) or (ra_max < ra_transient):
        continue
    if (dec_min > dec_transient) or (dec_max < dec_transient):
        continue
    if hdu[0].header['FILTER'] in filt_: continue

    ID = hdu[0].header['OBJECT']
    ...
    print(fitsfile, hdu[0].header['FILTER'], i)
    #continue
...
#           print('solve-field '+files[i][:-5]+'_red.fits --ra '+str(ra)+' --dec '+str(dec)+' --dir /home/roboao/Tomas/output --scale-units arcsecperpix --scale-low 0.255 --scale-high 0.26 --radius 0.04 --overwrite')
    ...
    if opts.doAstrometryNet:
        ztfsub.utils.astrometrynet(fitsfilenew,pixel_scale=0.185,ra=ra,dec=dec,radius=5.0,depth=100,cutedges=-1,ext=1)

    hdu = fits.open(fitsfilenew)
    data = hdu[1].data
    header = hdu[0].header

    if i == 20:
        print(hdu[0].header['FILTER'])
        hdu[1].header["CRVAL1"] = ra_transient
        hdu[1].header["CRVAL2"] = dec_transient
        hdu[1].header["CRPIX1"] = 1122
        hdu[1].header["CRPIX2"] = 4581
    elif i == 23:
        hdu[1].header["CRVAL1"] = ra_transient
        hdu[1].header["CRVAL2"] = dec_transient
        hdu[1].header["CRPIX1"] = 958
        hdu[1].header["CRPIX2"] = 4570
    elif i == 129:
        hdu[1].header["CRVAL1"] = ra_transient
        hdu[1].header["CRVAL2"] = dec_transient
        hdu[1].header["CRPIX1"] = 1122
    ...
    #cutout = Cutout2D(hdu[1].data[:-50,:], position=(x0, y0), size=size, wcs=w)

    # Put the cutout image in the FITS HDU
    #hdu[1].data = cutout.data

    # Update the FITS header with the cutout WCS
    #hdu[1].header.update(cutout.wcs.to_header())

    #x0, y0 = w.wcs_world2pix(ra_transient,dec_transient,1)
    #xmin, xmax = int(x0-250), int(x0+250)
    #ymin, ymax = int(y0-400), int(y0+300)

    #hdu0 = fits.PrimaryHDU(header=hdu[0].header)
    #hdu1 = fits.ImageHDU(header=hdu[1].header,data=hdu[1].data[ymin:ymax,xmin:xmax])
    #hdu1.header['NAXIS1'] = 500
    #hdu1.header['NAXIS2'] = 500
    ...
    #if opts.doAstrometryNet:
    #    ztfsub.utils.astrometrynet(fitsfilenew,pixel_scale=0.185,ra=ra,dec=dec,radius=1.0,depth=100,cutedges=-1,ext=1)

    if opts.doSubtraction:
        passband = hdu[0].header['FILTER'][0]
...
if opts.doDifferential:
    ps1_table_diff = ps1_query(opts.ra_diff, opts.dec_diff, 1.5/3600.0,maxsources=1, maxmag=23.0)

# do reduction
KPED = []
for i in range(len(files_entire)):
    if (not "20" in files_entire[i]) and (not "23" in files_entire[i]) and (not "129" in files_entire[i]): continue
    ...
    if filt == 'up':
        ...
        ps1_table = ps1_query(ra_header, dec_header, radius_deg,maxsources=3000, maxmag=23.0)
        gaia_table = gaia_query(ra_header, dec_header, radius_deg,maxsources=3000, maxmag=23.0)
    ...
    if opts.doSubtraction:
        ...
        c = plt.pcolor(X,Y,cutout.data.T,vmin=-100,vmax=100)
        fig.colorbar(c) 
        ...
        if opts.doDifferential:
            ...
            x0, y0 = w.wcs_world2pix(opts.ra_diff,opts.dec_diff,1)
            cutout = Cutout2D(data, position=(x0, y0), size=size, wcs=w)
            cutout.data = cutout.data - np.median(cutout.data)
            sizex, sizey = int(cutout.shape[0]/2.0), int(cutout.shape[1]/2.0)
            x, y = np.arange(-sizex, sizex), np.arange(-sizey, sizey)
            Y, X = np.meshgrid(x,y)
