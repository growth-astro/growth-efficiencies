...
def parse_commandline():
    ...
    parser.add_option("-o", "--outputDir", help="output file",default="../output/GW190901/")
    parser.add_option("-l","--transients", help="Transient list.", default='../data/GW190901/transients.dat')
    parser.add_option("-t","--transient", help="Transient name.", default='ZTF19abvizsw')
    parser.add_option("-d","--dataDir", help="Data directory.", default='../data/GW190901/ZTF19abvizsw/') 
    parser.add_option("-s","--sextractorDir", help="sextractor directory.", default='../sextractor/')
    parser.add_option("--defaultsDir",default="../defaults")
    parser.add_option("-p","--pixel_scale", help="pixel scale (1m=0.389, 2m=0.3).", default=0.389)    
...
def stack_images(scienceimage,fitsfiles,doRegistration=False):
    ...
    for ii in range(len(fitsfiles)):
        ...
        if doRegistration:
            ...
            shift, error, diffphase = register_translation(reference, hdulist[1].data, upsample_factor=1)
            shiftx, shifty = shift[0], shift[1]

            shifted = image_registration.fft_tools.shiftnd(hdulist[1].data, (shiftx,shifty))
            data = np.append(data,np.expand_dims(shifted,axis=2),axis=2)
        else:
            data = np.append(data,np.expand_dims(hdulist[1].data,axis=2),axis=2)
...
files_old = glob.glob(workdir_fits_red+'*91.fits*')
print(files_old)#,workdir_fits_red


filter_folders = ['g','r','i','u']
if len(workdir_fits_red.split("/")[-2])>2 or len(workdir_fits_red.split("/")[-1])>2:
    for filter_folder in filter_folders:
        if not os.path.isdir(workdir_fits_red+filter_folder):
            os.makedirs(workdir_fits_red+filter_folder)

## add stacking code
if opts.doStack:
    stackDir = os.path.join(workdir_fits_red,'stack')
    ...
    for i in range(len(files_old)):
        ...
        filt = hdu[1].header['filter']
        if not filt in stacks:
            stacks[filt] = []
        stacks[filt].append(fitsfile)

    for filt in stacks.keys():
        fitsfile = os.path.join(stackDir,'stack_%s.fits' % filt)
        stack_images(fitsfile,stacks[filt],doRegistration=True)
    files_old = sorted(glob.glob(os.path.join(stackDir,'*.fits*')))
...
for i in range(len(files_old)):
    ...
    for ii in range(len(hdu)):
        if 'OBJECT' in list(hdu[ii].header.keys()):
            OBJECT = ii
            print('OBJECT assigned',ii)
        elif 'XTENSION' in list(hdu[ii].header.keys()):
            if hdu[ii].header['XTENSION'] == 'BINTABLE':
                BINTABLE = ii
                print('bintable assigned',ii)
            if hdu[ii].header['XTENSION'] == 'IMAGE':
                IMAGE = ii
                print('image assigned',ii)
        else:
            OTHER = ii
            print('other assigned',ii)

	
    ID = hdu[OBJECT].header['OBJECT']
    ra_header_.append(hdu[OBJECT].header['CRVAL1'])
    dec_header_.append(hdu[OBJECT].header['CRVAL2'])
    filt_.append(hdu[OBJECT].header['filter'])
    date = Time(hdu[OBJECT].header['DATE-OBS'], format='isot', scale='utc')
    date_.append(date)
    date_jd_.append(date.jd)
    name_.append(ID)
	
    os.system('cp '+fitsfile+' '+workdir_fits_red+hdu[OBJECT].header['filter'][0])
...
#           print('solve-field '+files[i][:-5]+'_red.fits --ra '+str(ra)+' --dec '+str(dec)+' --dir /home/roboao/Tomas/output --scale-units arcsecperpix --scale-low 0.255 --scale-high 0.26 --radius 0.04 --overwrite')
    ...
    hdu0 = fits.PrimaryHDU(header=hdu[OBJECT].header,data=hdu[OBJECT].data)
    #hdu1 = fits.ImageHDU(header=hdu[OBJECT].header,data=hdu[IMAGE].data)

    #del hdu[0].header["EXTEND"]
    #hdulist = fits.HDUList(hdus=[hdu0,hdu1])

    fitsfilenew = data_path+'/image_'+str(i)+'.fits'
    hdu0.writeto(fitsfilenew,overwrite=True)
    ...
    if opts.doSubtraction:
        passband = hdu[OBJECT].header['filter'][0]
...
for i in range(len(files_entire)):
    ...
    ra,dec,mag_psf,mag_psf_err,x,y,SNR = cat['ALPHA_J2000'] , cat['DELTA_J2000'],cat['MAG_BEST'],cat['MAGERR_BEST'],cat['X_IMAGE'],cat['Y_IMAGE'],cat['FLUX_BEST']/(cat['BACKGROUND'])
    if opts.doSubtraction:
        ra_sub,dec_sub,mag_psf_sub,mag_psf_err_sub,x_sub,y_sub,SNR = catsub['ALPHA_J2000'] , catsub['DELTA_J2000'],catsub['MAG_BEST'],catsub['MAGERR_BEST'],catsub['X_IMAGE'],catsub['Y_IMAGE'],cat['FLUX_BEST']/cat['BACKGROUND'] 
    ...
    if opts.doDifferential: 
        ...
        for ii in range(len(ind_im)):
            if ind_im[ii] < len(ps1_table) and np.abs(mag_psf_err[ii])<0.15 :
                f = filt[0]+'mag'
    ...
    maglim = ZP_median+mag_psf[np.argsort(SNR)[np.sort(SNR)<3][-1]] # np.max(mag_psf[mag_psf_err<0.11])
    sum_phrase = ''
    if opts.doSubtraction:
        ...
        if match_tra_sub.any():
            ...
            sum_list = [name_[i],date_jd_[i],ra_all_tran[i],dec_all_tran[i],filt[0],'=',np.round(mag_psf_sub+ZP_median,3),'+-',str(np.round(mag_psf_err_tot,2)),np.round(maglim,2)]
            sum_phrase = ' '.join([str(i) for i in sum_list])
            print(sum_phrase)
        else: 
            ...
            sum_list = [name_[i],date_jd_[i],ra_transient,dec_transient,filt[0],'>',np.round(maglim,2),'+-',str(np.round(ZP_err,2))]
            sum_phrase = ' '.join([str(i) for i in sum_list])
            print(sum_phrase)
    else:
        if match_tra.any():
            ...
            sum_list =  [name_[i],date_jd_[i],ra_all_tran[i],dec_all_tran[i],filt[0],'=',np.round((mag_psf[ind_tra][0]+ZP_median),3),'+-',str(np.round(mag_psf_err[ind_tra][0],2)),np.round(maglim,2)]
            sum_phrase = ' '.join([str(i) for i in sum_list])
            print(sum_phrase)
        else: 
            print('No transient detected',filt,'  = ',np.round(maglim,3))
    ...
    if opts.doSubtraction:
        ...
        plotName = os.path.join(outputDir,'image_all_%s.pdf' % (filt))
        fig = plt.figure(figsize=(15,7))
        f1 = aplpy.FITSFigure(refimage, figure=fig, subplot=[0.1,0.3,0.29,0.59])
        f1.show_grayscale(stretch='power',pmin=5,pmax=95)
        f1.show_circles(ra,dec,1.5/3600.0,zorder=99,linestyle='dashed', edgecolor='red')
        f1.show_circles(ps1_table['RAJ2000'],ps1_table['DEJ2000'],3.0/3600.0,zorder=99,linestyle='dashed', edgecolor='green')
        f1.show_circles(ra_transient,dec_transient,1.5/3600.0,zorder=99,linestyle='dashed', edgecolor='blue')
        ...
        f1.set_title('REF'+filt)

        f2 = aplpy.FITSFigure(imagefilesub, figure=fig, subplot=[0.39,0.3,0.29,0.59])
        f2.show_grayscale(stretch='power',pmin=5,pmax=95)
#         f2.show_circles(ra_sub,dec_sub,1.5/3600.0,zorder=99,linestyle='dashed', edgecolor='red')
#         f2.show_circles(ps1_table['RAJ2000'],ps1_table['DEJ2000'],3.0/3600.0,zorder=99,linestyle='dashed', edgecolor='green')
        f2.show_circles(ra_transient,dec_transient,3.5/3600.0,zorder=99,linestyle='dashed', edgecolor='blue')
        f2.axis_labels.set_xtext('Right Ascension')
        f2.axis_labels.set_ytext('Declination')
        xpix, ypix = f2.world2pixel(ra_transient,dec_transient)
        f2._ax1.set_xlim(xpix-100,xpix+100)
        f2._ax1.set_ylim(ypix-100,ypix+100)
        f2.show_grayscale(stretch='power',pmin=5,pmax=95)
        fig.canvas.draw()
        ...
        f2.set_title('SUB'+filt)

        f3 = aplpy.FITSFigure(cutout, figure=fig, subplot=[0.68,0.3,0.29,0.59])
        f3.show_grayscale(stretch='power',pmin=5,pmax=95)
        ...
        radii = [2*fwhm,3*fwhm,4*fwhm]
        for radius in radii:
            plt.plot(radius*np.cos(angles)+25,radius*np.sin(angles)+25,'yellow')
        f3.set_title('LCO'+filt)
    ...
    plotName = os.path.join(outputDir,'summary_%s_%d.pdf' % (filt, i))
    fig = plt.figure(figsize=(7,10))
    f1 = aplpy.FITSFigure(imagefile, figure=fig, subplot=[0.1,0.35,0.9,0.65])
    #f1.show_grayscale(invert=True)
    f1.show_grayscale(stretch='power',pmin=5,pmax=95)
    #idx = np.where(mag_psf_err<0.05)[0]
    f1.show_circles(ra,dec,1.5/3600.0,zorder=99,linestyle='dashed', edgecolor='red')
    ...
    if x0>0 and y0>0:
        ...
        f3 = aplpy.FITSFigure(cutout, figure=fig, subplot=[0.1,0.05,0.5,0.3])
        f3.show_grayscale(stretch='power',pmin=5,pmax=95)
        f3.set_tick_labels_font(size='x-small')
        f3.set_axis_labels_font(size='small')
        f3.hide_yaxis_label()
        ...
        for radius in radii:
            plt.plot(radius*np.cos(angles)+25,radius*np.sin(angles)+25,'yellow')
        names_sum = ['name:','date_jd:','ra:','dec:','filt:',' ','mag_psf:','','err:','maglim:']
        for iii in range(len(sum_list)):
    
            plt.text(55,35-3*iii ,names_sum[iii]+str(sum_list[iii]), fontsize=12)
        ...
        warnings.warn("Warning.......... TRANSIENT OUT OF THE IMAGE\n"+names_sum[iii])
...
for image, det, filt, mag, magerr in KPED:
    fid.write('%s %s %s %.5f %.5f\n' % (image, dett[det], filt, mag, magerr))
    print(('%s_%s %s %.3f %.3f\n' % (ID, filt ,dett[det], mag, magerr)))
fid.close()
