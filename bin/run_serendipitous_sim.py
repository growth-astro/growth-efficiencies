...
if opts.modelType == "Bulladynwind":
    ...
    outputDirs = glob.glob(os.path.join(baseoutputDir, '*_*_*'))
...
for outputDir in outputDirs:
    ...
    if opts.modelType == "Bulladynwind":
        ...
        jdstart, jdend, mag, dmag = np.array(outsplit, dtype=float)
    else:
...
if opts.modelType == "Tophat":
    ...
    models = [['GW170817', 0.5, 0.5, -16.0, -16.0]]
    ...
    #c = plt.pcolor(X,Y,Z,norm=LogNorm(),vmin=1e-3,vmax=0.5,cmap='coolwarm')
    c = plt.pcolor(X,Y,Z,vmin=1e-3,vmax=0.5,cmap='coolwarm')
    cbar = fig.colorbar(c)
