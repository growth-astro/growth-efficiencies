...
files_old = glob.glob(workdir_fits_red+'*91*')
...
for i in range(len(files_old)):
    ...
    ra_header_.append(hdu[1].header['CRVAL1'])
    dec_header_.append(hdu[1].header['CRVAL2'])
    filt_.append(hdu[1].header['filter'])
    date = Time(hdu[1].header['DATE-OBS'], format='isot', scale='utc')
    date_.append(date)
...
#           print('solve-field '+files[i][:-5]+'_red.fits --ra '+str(ra)+' --dec '+str(dec)+' --dir /home/roboao/Tomas/output --scale-units arcsecperpix --scale-low 0.255 --scale-high 0.26 --radius 0.04 --overwrite')
    ...
    if opts.doSubtraction:
        passband = hdu[1].header['filter'][0]
